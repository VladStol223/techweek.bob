(function() {
  ...
  // Step 6: copy the track-specific audit prompt, then show LinkedIn popup after 2s
  var liPopupTimer = null;
  var liPopupOverlay = document.getElementById('li-popup-overlay');
  var liPopupPost    = document.getElementById('li-popup-post');
  var liPopupCopy    = document.getElementById('li-popup-copy-btn');
  var liPopupClose   = document.getElementById('li-popup-close');

  var LI_POSTS = {
    'card-a': "Just earned my IBM Bob badge at Atlanta Tech Week — Americas Mart, Atlanta! 🏅\n\nI ran a full AI-powered engineering audit on a real codebase using IBM Bob's Engineering Audit skill. 9 autonomous stages: discovery, triage, remediation, verification, validation, and a final release verdict — all driven by AI.\n\nAI isn't just writing code anymore. It's reading entire codebases, classifying risk, implementing fixes, and shipping release reports. That future is here.\n\nThanks to IBM and the Americas Mart team for an incredible session. If you haven't explored IBM Bob yet, now is the time.\n\n#IBMBob #AtlantaTechWeek #AI #SoftwareEngineering #IBM",
    'card-b': "Just earned my IBM Bob badge at Atlanta Tech Week — Americas Mart, Atlanta! 🏅\n\nI built a fully interactive supply chain operations dashboard — live map, animated delivery trucks, drill-down store floor plans, and a delivery timeline — all generated by IBM Bob from a single dataset.\n\nNo templates. No boilerplate. Just a prompt, a JSON file, and an AI that knows how to ship.\n\nThanks to IBM and the Americas Mart team for an inspiring afternoon. Incredibly excited about what's possible with AI-assisted development.\n\n#IBMBob #AtlantaTechWeek #AI #SupplyChain #IBM",
    'card-c': "Just earned my IBM Bob badge at Atlanta Tech Week — Americas Mart, Atlanta! 🏅\n\nI completed a guided AI learning session using IBM Bob — picking a mentor mode (Python, SQL, Quantum, or Language) and going through a real, adaptive lesson built around my level and goals.\n\nLearning with AI feels completely different. It meets you where you are, adjusts in real time, and keeps you accountable. This is what personalized education looks like.\n\nThanks to IBM and the Americas Mart team for making this possible!\n\n#IBMBob #AtlantaTechWeek #AI #Learning #IBM",
    'card-d': "Just earned my IBM Bob badge at Atlanta Tech Week — Americas Mart, Atlanta! 🏅\n\nI used IBM Bob's discovery quiz to find exactly where AI fits into my workflow — answered three questions about my role, my goals, and how I like to work, and got a custom prompt tailored to me.\n\nThe most underrated part of AI adoption isn't the tools — it's knowing where to start. Today I found my starting point.\n\nThanks to IBM and the Americas Mart team for a genuinely useful workshop!\n\n#IBMBob #AtlantaTechWeek #AI #Productivity #IBM",
    'card-e': "Just earned my IBM Bob badge at Atlanta Tech Week — Americas Mart, Atlanta! 🏅\n\nI brought my own real-world problem to the IBM Bob Bobathon and used the Bob Marketplace to find, install, and run the right skill or mode for it — then worked through it with AI from scratch.\n\nThe moment AI solves a problem you actually care about, everything clicks. That happened for me today.\n\nThanks to IBM and the Americas Mart team for an energizing event. Highly recommend for anyone curious about putting AI to work.\n\n#IBMBob #AtlantaTechWeek #AI #IBM #Innovation"
  };

  function showLiPopup() {
    var track = currentBadgeTrack || '';
    var post  = LI_POSTS[track] || LI_POSTS['card-a'];
    liPopupPost.textContent = post;
    liPopupOverlay.classList.add('li-popup-visible');
    liPopupClose.focus();
    gtag('event', 'li_popup_shown', { track_id: track });
  }
  function hideLiPopup() {
    liPopupOverlay.classList.remove('li-popup-visible');
  }

  liPopupClose.addEventListener('click', hideLiPopup);
  liPopupOverlay.addEventListener('click', function(e) {
    if (e.target === liPopupOverlay) hideLiPopup();
  });
  document.addEventListener('keydown', function(e) {
    if (e.key === 'Escape' && liPopupOverlay.classList.contains('li-popup-visible')) hideLiPopup();
  });

  liPopupCopy.addEventListener('click', function() {
    var text = liPopupPost.textContent || '';
    var originalHTML = liPopupCopy.innerHTML;
    function onCopied() {
      liPopupCopy.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" width="14" height="14"><polyline points="20 6 9 17 4 12"/></svg> Copied!';
      liPopupCopy.classList.add('li-copied');
      setTimeout(function() { liPopupCopy.innerHTML = originalHTML; liPopupCopy.classList.remove('li-copied'); }, 2200);
      gtag('event', 'li_post_copied', { track_id: currentBadgeTrack || 'unknown' });
    }
    if (navigator.clipboard) {
      navigator.clipboard.writeText(text).then(onCopied).catch(function() { fallbackCopy(text); onCopied(); });
    } else { fallbackCopy(text); onCopied(); }
  ...
  // Wire up the audit copy button — show LinkedIn popup 2s after copy
  var auditCopyBtn = document.getElementById('badge-copy-audit-btn');
  auditCopyBtn.addEventListener('click', function() {
    var text = document.getElementById('badge-audit-prompt-text').value;
    var originalHTML = auditCopyBtn.innerHTML;
    function onCopied() {
      auditCopyBtn.innerHTML = '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" style="width:14px;height:14px;flex-shrink:0"><polyline points="20 6 9 17 4 12"/></svg> Copied!';
      auditCopyBtn.classList.add('copied');
      setTimeout(function() {
        auditCopyBtn.innerHTML = originalHTML;
        auditCopyBtn.classList.remove('copied');
      }, 2200);
      gtag('event', 'badge_prompt_copy', { label: 'track-audit' });
      // Show LinkedIn popup after a 2-second delay
      clearTimeout(liPopupTimer);
      liPopupTimer = setTimeout(showLiPopup, 2000);
    }
    if (navigator.clipboard) {
      navigator.clipboard.writeText(text).then(onCopied).catch(function() { fallbackCopy(text); onCopied(); });
    } else { fallbackCopy(text); onCopied(); }
